...
def test_remove_nans_and_subsample(df_positive_skew):
    ...
    cleaned, sampled, subsampled = remove_nans_and_subsample(df, 'feature', 
                                        verbose=False, subsample_limit=500)
...
def test_apply_transformations(df_positive_skew):
    ...
    transformed, results = apply_transformations(df, 'feature', 
                                initial_skew=skew, max_power=10, plot=False)
    assert isinstance(transformed, dict)
    assert all(isinstance(v, np.ndarray) or isinstance(v, pd.Series) 
...
def test_apply_final_transformation(df_positive_skew):
    ...
    result_df = apply_final_transformation(df, transformed, 'log', 'feature',
                                 subsampled=False, positively_skewed=True)
...
def test_correct_skew_positive(df_positive_skew):
    ...
    """
    df = df_positive_skew.copy()
    df_out, results = correct_skew(df, 'feature', 
