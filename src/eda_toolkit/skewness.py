...
def remove_nans_and_subsample(df: pd.DataFrame, 
    ...
    if verbose:
        logging.info(f"Removed {initial_len - len(df)} "
                      f"rows with NaN in {feature}")

    if len(df) > subsample_limit:
        df_sampled = df.sample(n=subsample_limit, random_state=42)
        if verbose:
            logging.info(f"Subsampled to {len(df_sampled)} rows "
                          f"from {len(df)}")
...
def apply_transformations(
    ...
    if initial_skew > skew_threshold:
        ...
        power_min_skew = min(skew_power_transformation, 
        ...
        transformed[feature_power_name] = np.power(df_temp[feature], 
                                                   1/power_min_skew)
        results[feature_power_name] = abs(round(best_skew, 4))
        skewness_power_plot_title = f'Skewness after raising to ' 
        ...
        skewness_log_plot_title = f'Skewness after log transformation: '
        skewness_log_plot_title += f'{round(log_skew, 4)}'

        feature_yeo_name = 'yeo'
        pwr = PowerTransformer(method='yeo-johnson')
        transformed[feature_yeo_name] = pwr.fit_transform(df_temp[[feature]])
        transformed[feature_yeo_name] = transformed[feature_yeo_name].flatten()
        yeo_skew = pd.Series(transformed[feature_yeo_name]).skew()
        results[feature_yeo_name] = abs(yeo_skew)
        skewness_yeo_plot_title = f'Skewness after Yeo-Johnson transformation: '
        ...
        skewness_power2_plot_title = f'Skewness after raising to 2: ' 
        ...
        skewness_power3_plot_title = f'Skewness after raising to 3: '
...
def apply_final_transformation(df: pd.DataFrame, 
    ...
    #applied to the entire dataset. Otherwise, the transformations were applied 
...
def correct_skew(df, feature, max_power=50, 
    ...
    if skew < initial_skew_threshold:
        if verbose:
            logging.info("Feature not sufficiently skewed. No transformation " \
                 "applied.")
            return df_temp
    ...
    best_transformation, success = select_best_transformation(results, 
                                                     final_skew_threshold)

    if not success:
        if verbose:
            logging.info(f"Could not reduce skewness below threshold. "
                         f"Converting '{feature}' to binary.")
        df_out = fallback_to_binary(df, feature, skew_positive=(skew > 0))
    else:
        df_out = apply_final_transformation(df, transformed, 
                                            best_transformation, 
                                            feature, 
                                            subsampled, 
    ...
    if plot_transformed_feature:
        ...
        if success:
            sns.histplot(df_out[feature + '_transformed'], kde=True,
                          stat="density", ax=axs[1])
...
if __name__ == "__main__":
    ...
    airline_tranformed, resultados = correct_skew(airline,
